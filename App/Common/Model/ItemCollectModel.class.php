...
class ItemCollectModel extends Model {
	...
	public function loadList($where = array(), $limit = 10 ,$order = 'time desc'){
		return 	M('item_collect as a')
				...
				->field('a.*,b.name as item_name,b.cover_img, b.desc,b.raising_money,b.investment_rate,b.lowest_money,c.name as progress_name,d.name as user_name, header')
				->where($where)
				->limit($limit)
				->order($order)
				->select();
	...
	//获取用户收藏项目数量
	public function getusercollectnum($where){
         return M('item_collect')->where($where)->count();
	}
