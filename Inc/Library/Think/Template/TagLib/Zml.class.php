...
Eof;
	...
	public function _newslist($attr,$content){
		...
		$str .= '$info = D("News")->loadList("'.$w.'","'.$limit.'","'.$order.'");';
		$str .= 'foreach ($info as $i=>$zml):';
    ...
    public function _nav($attr,$content){
        ...
        $str .= '$info = D("Nav")->loadList("'.$w.'","'.$limit.'");';
        $str .= 'foreach ($info as $i=>$zml):';
        $str .= '$nav2=D("Nav")->loadList(array("pid"=>$zml["id"]));';
        $str .= '?>';    
        $str .= $this->tpl->parse($content);
        $str .='<?php endforeach ?>'; 
        return $str;
    ...
    public function _itemlist($attr,$content){
        ...
        if($item_type){
            $w = ($w) ? $w.' AND a.item_type ='.$item_type : 'a.item_type ='.$item_type;
        }
        //属性条件
        if($attr == 1){
        ...
        $str .= '$itemList = D("Item")->loadList("'.$w.'","'.$limit.'","'.$order.'");';
        $str .= 'foreach ($itemList as $i=>$zml):';
        if($item_type==1){
            $str .= '$zml["url"]=U("item/info",array("id"=>$zml["id"]));';
        }else{
            $str .= '$zml["url"]=U("product/detail",array("id"=>$zml["id"]));';
        }

        $str .= '$zml["last_time"]=gettime($zml["end_time"]-time());';
        $str .= '$zml["success_rate"]=D("ItemWithOrder")->successRate($zml["id"]);';
        $str .= '$zml["count_money"]=D("ItemWithOrder")->countMoney($zml["id"]);';
        $str .= '?>';    
        $str .= $this->tpl->parse($content);
        $str .='<?php endforeach ?>'; 
        return $str;
